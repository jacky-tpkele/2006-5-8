  ...
  try {
    ...
    if (slug) {
      ...
      const blogPost = {
        ...
        articleType: post.article_type || null,
        date: post.published_at?.split('T')[0] || new Date().toISOString().split('T')[0],
        image: post.cover_image_url || post.image_url || '/assets/blog/default.webp',
        coverImageAlt: post.cover_image_alt || post.title,
        excerpt: post.content.split('\n').find((l: string) => l.trim() && !l.startsWith('#'))?.slice(0, 150) || post.content.slice(0, 150),
