...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const s = getStandardBySlug(slug);
  if (!s) return {};
...
export default async function Detail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const s = getStandardBySlug(slug);
  if (!s) notFound();
  const rel = getRelatedStandards(slug);
    ...
    <>
      ...
      <main className="standardDetail">
        <div className="container">
          ...
          <div className="detailHero">
            <div>
              <div className="eyebrow">
                {s.system} · {s.referenceType}
              </div>
              <h1>{s.code}</h1>
              <p>{s.title}</p>
            </div>
            <a className="primaryButton" href={s.source} target="_blank" rel="noreferrer">
              Open Official Source ↗
            </a>
          </div>

          <div className="detailMetrics">
            <Metric label="Organization" value={s.system} />
            <Metric label="Edition / Review Year" value={String(s.year)} />
            <Metric label="Status" value={s.status} />
            <Metric label="Reference Type" value={s.referenceType} />
            <Metric label="TPKELE Relevance" value={s.relevance} />
          </div>

          <div className="detailLayout">
            <article>
              <Block title="What This Standard Covers">
                <p>{s.scope}</p>
              </Block>

              <Block title="Why It Matters to Buyers">
                <ul>
                  {s.why.map((x) => (
                    <li key={x}>{x}</li>
                  ))}
              ...
              <Block title="Relevant TPKELE Products">
                <div className="tagWrap">
                  {s.products.map((p) => (
                    <span className="productTag" key={p}>
                      {p}
                    </span>
                  ))}
                </div>
              ...
              <Block title="Related Standards / Notes">
                <ul>
                  {s.related.map((x) => (
                    <li key={x}>{x}</li>
                  ))}
            ...
            <aside>
              <div className="scopeWarning">{s.warning}</div>
          ...
          {rel.length > 0 && (
            <section className="relatedStandards">
