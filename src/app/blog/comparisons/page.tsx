...
export default async function ComparisonsPage() {
  ...
  const filtered = (allPosts as any[]).filter((p) => p.articleType === "comparison");
  ...
  return (
    <main>
      <PageTitle title="Comparisons" crumb="Blog · Comparisons" />
      <section className="section">
        <p style={{ color: "var(--muted)", marginBottom: 18, maxWidth: 760 }}>
          {BLOG_CATEGORY_DESCRIPTIONS["comparisons"]}
        </p>
        ...
        <nav aria-label="Blog categories" className="category-strip" style={{ marginBottom: 24 }}>
          ...
          {BLOG_CATEGORY_ORDER.map((slug) => (
            <Link
              key={slug}
              href={`/blog/${slug}`}
              className={slug === "comparisons" ? "category-chip active" : "category-chip"}
            >
