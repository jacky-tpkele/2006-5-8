...
export default async function FaqsPage() {
  ...
  const filtered = (allPosts as any[]).filter((p) => p.articleType === "faq");
  ...
  return (
    <main>
      <PageTitle title="FAQs" crumb="Blog · FAQs" />
      <section className="section">
        <p style={{ color: "var(--muted)", marginBottom: 18, maxWidth: 760 }}>
          {BLOG_CATEGORY_DESCRIPTIONS["faqs"]}
        </p>
        ...
        <nav aria-label="Blog categories" className="category-strip" style={{ marginBottom: 24 }}>
          ...
          {BLOG_CATEGORY_ORDER.map((slug) => (
            <Link
              key={slug}
              href={`/blog/${slug}`}
              className={slug === "faqs" ? "category-chip active" : "category-chip"}
            >
