...
export default async function ProductKnowledgePage() {
  ...
  const filtered = (allPosts as any[]).filter((p) => p.articleType === "product");
  ...
  return (
    <main>
      <PageTitle title="Product Knowledge" crumb="Blog · Product Knowledge" />
      <section className="section">
        <p style={{ color: "var(--muted)", marginBottom: 18, maxWidth: 760 }}>
          {BLOG_CATEGORY_DESCRIPTIONS["product-knowledge"]}
        </p>
        ...
        <nav aria-label="Blog categories" className="category-strip" style={{ marginBottom: 24 }}>
          ...
          {BLOG_CATEGORY_ORDER.map((slug) => (
            <Link
              key={slug}
              href={`/blog/${slug}`}
              className={slug === "product-knowledge" ? "category-chip active" : "category-chip"}
            >
