...
export default async function ApplicationScenariosPage() {
  ...
  const filtered = (allPosts as any[]).filter((p) => p.articleType === "application");
  ...
  return (
    <main>
      <PageTitle title="Applications" crumb="Blog · Applications" />
      <section className="section">
        <p style={{ color: "var(--muted)", marginBottom: 18, maxWidth: 760 }}>
          {BLOG_CATEGORY_DESCRIPTIONS["application-scenarios"]}
        </p>
        ...
        <nav aria-label="Blog categories" className="category-strip" style={{ marginBottom: 24 }}>
          ...
          {BLOG_CATEGORY_ORDER.map((slug) => (
            <Link
              key={slug}
              href={`/blog/${slug}`}
              className={slug === "application-scenarios" ? "category-chip active" : "category-chip"}
            >
